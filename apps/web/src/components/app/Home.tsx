...
export function Home() {
  ...
  const { data: albumsData, isLoading: albumsLoading } = useLibraryAlbums({ limit: 6 });
  const { data: artistsData, isLoading: artistsLoading } = useLibraryArtists(1, 6);
  const { data: playlistsData, isLoading: playlistsLoading } = useLibraryPlaylists();
  const { data: tracksData, isLoading: tracksLoading } = useLibraryTracks({ limit: 6 });
  ...
  const playlists = playlistsData?.data?.items?.slice(0, 6) ?? [];
  const artists = artistsData?.data?.items?.slice(0, 6) ?? [];
  const albums = albumsData?.data?.items?.slice(0, 6) ?? [];
  const tracks = tracksData?.data?.items?.slice(0, 6) ?? [];
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 min-w-0">
                    {artists.map((a, i) => (
                      <span key={a.id} className="flex items-center">
                        ...
                        {i < artists.length - 1 && <span className="mr-1">,</span>}
                      </span>
                    ))}
                    {album && (
                      <>
