...
export function SearchInput({ className, initialValue = '', onSearch, lockedScope }: SearchInputProps) {
  ...
  const globalSuggestions = useSearchSuggestions(
    ...
    { enabled: currentScope === 'all' && isOpen && debouncedQuery.trim().length >= 3 },
  );

  const librarySuggestions = useLibrarySearchSuggestions(
    { query: debouncedQuery },
    { enabled: currentScope === 'library' && isOpen && debouncedQuery.trim().length >= 3 },
  );

  const { data: response, isLoading, isFetching, error } = currentScope === 'all' ? globalSuggestions : librarySuggestions;
  ...
  return (
    <div ref={containerRef} className={cn('flex items-center gap-2 w-full max-w-2xl', className)}>
      <div className="relative flex-1">
        ...
        <Input
          ...
          placeholder={currentScope === 'all' ? "Search on Playr..." : "Search in your library..."}
          className="pl-9 pr-9 bg-stone-900/50 border-white/10 focus:bg-stone-900 transition-all rounded-xl h-10"
        />
        {isFetching && (
                      ...
                      onClick={() => {
                        ...
                        navigate({
                          ...
                          search: (prev: any) => ({ 
                            ...prev, 
                            query: result.name,
                            filters: { ...prev.filters, visibility: currentScope === 'library' ? 'private' : 'public' }
                          }),
                        });
                      }}
      ...
      <SearchScopeToggle lockedScope={lockedScope} />
    </div>
  );
}
