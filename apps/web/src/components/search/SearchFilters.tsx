...
export function SearchFilters({ search, navigate }: SearchFiltersProps) {
  return (
    <div className="p-6 bg-stone-900/50 border border-white/10 rounded-xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-in slide-in-from-top-2 duration-200">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2">
            <Checkbox
              id="verified-only"
              checked={!!(search.filters?.artist?.verified || search.filters?.track?.verified || search.filters?.album?.verified)}
      ...
      {(!search.filters?.categories || search.filters.categories.includes('track')) && (
        <div className="flex flex-col gap-3">
          <span className="text-xs font-semibold uppercase text-muted-foreground">Track Filters</span>
          <div className="space-y-2">
             ...
             <Slider
               ...
               onChange={(val) => navigate({
                 search: (prev: any) => ({ ...prev, filters: { ...prev.filters, track: { ...prev.filters?.track, durationTo: val } } })
               })}
             />
          </div>
        </div>
      ...
      {(!search.filters?.categories || search.filters.categories.includes('album')) && (
        <div className="flex flex-col gap-3">
          <span className="text-xs font-semibold uppercase text-muted-foreground">Album Type</span>
          <Select
            value={search.filters?.album?.type || 'none'}
            onValueChange={(val) => navigate({
              search: (prev: any) => ({ ...prev, filters: { ...prev.filters, album: { ...prev.filters?.album, type: val === 'none' ? undefined : (val as any) } } })
            })}
          >
