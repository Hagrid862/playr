...
import { AddAlbumToPlaylistDropdownSubmenu } from './AddAlbumToPlaylistDropdownSubmenu';
...
vi.mock('@/components/ui/dropdown-menu', () => ({
  DropdownMenuItem: ({ children, onClick, disabled }: ComponentPropsWithoutRef<'button'>) => (
    <button type="button" onClick={onClick} disabled={disabled}>
      {children}
    </button>
  ),
  DropdownMenuPortal: ({ children }: PropsWithChildren) => <>{children}</>,
  DropdownMenuSeparator: () => <hr data-testid="menu-separator" />,
  DropdownMenuSub: ({ children }: PropsWithChildren) => (
    <div data-testid="mock-dropdown-sub">{children}</div>
  ),
  DropdownMenuSubContent: ({ children }: PropsWithChildren) => (
    <div data-testid="mock-dropdown-sub-content">{children}</div>
  ),
  DropdownMenuSubTrigger: ({ children }: PropsWithChildren) => (
...
describe('AddAlbumToPlaylistDropdownSubmenu', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockPlaylists.current = defaultPlaylistsResponse();
  });

  it('renders dropdown menu items properly with system roles and custom covers', () => {
    const { container } = customRender(<AddAlbumToPlaylistDropdownSubmenu albumId="album-123" />);
  ...
  it('handles empty playlists items array safely', () => {
    ...
    customRender(<AddAlbumToPlaylistDropdownSubmenu albumId="album-123" />);
