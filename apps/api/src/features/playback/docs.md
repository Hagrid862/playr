...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `query:get-state`                | Omit or `{}`                                                                                                             | [`GetPlaybackStateResponse`](../../../../../packages/contracts/src/playback/response/get-playback-state.response.ts) (`PlaybackState` **or `null`**) | **`null`** when there is no Redis document **or** when `updatedAt` is older than `PLAYBACK_STATE_STALE_AFTER_MS` (the key is then deleted).                              |
| `command:presence-touch`         | Omit or `{}`                                                                                                             | [`PresenceTouchResponse`](../../../../../packages/contracts/src/playback/response/presence-touch.response.ts)                                        | Refreshes the caller’s row in the playback device registry (extends the 90s TTL). Idempotent.                                                                            |
...
| `command:set-current-time-state` | [`SetCurrentTimeStateRequest`](../../../../../packages/contracts/src/playback/request/set-current-time-state.request.ts) | `PlaybackState`                                                                                                                                      | Partial update. **`expectedVersion`** must match current server `version` (**not** `0`). Can be sent from any connected session to move shared position for all clients. |
...
| `command:set-active-device`      | [`SetActiveDeviceRequest`](../../../../../packages/contracts/src/playback/request/set-active-device.request.ts)          | `PlaybackState`                                                                                                                                      | Sets `activeDeviceId` and active device metadata. Requires a known device and matching `expectedVersion`.                                                                |
| `query:list-devices`             | Omit or `{}`                                                                                                             | [`ListPlaybackDevicesResponse`](../../../../../packages/contracts/src/playback/response/list-playback-devices.response.ts)                           | Lists connected devices for the user, including `isActive` and `isCurrentDevice` flags.                                                                                  |
...
| `event:playback-session-ended` | `{}`            | When persisted playback for the user was removed server-side (e.g. last remaining track deleted). Clients should clear local session playback.   |
...
`PlaybackState.activeDeviceId` is authoritative for audio output ownership. Clients should only play local audio when it matches their local playback device ID.
