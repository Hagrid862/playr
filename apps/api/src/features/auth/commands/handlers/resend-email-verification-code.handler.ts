...
export class ResendEmailVerificationCodeHandler implements ICommandHandler<ResendEmailVerificationCodeCommand> {
  ...
  async execute(
    command: ResendEmailVerificationCodeCommand,
  ): Promise<ResendEmailVerificationCodeData> {
    ...
    const isEmailSent = await this.emailAuthService.beginEmailVerification(emailObject);
